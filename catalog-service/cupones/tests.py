...
class CuponValidacionTest(APITestCase):
    ...
    def test_rechaza_cupon_inactivo(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertFalse(response.data['valid'])
        self.assertIn('desactivado', response.data['message'].lower())
        self.assertEqual(response.data['discount'], 0)
    ...
    def test_calcula_descuento_porcentaje_correctamente(self):
        ...
        self.assertEqual(float(response.data['discount']), 20.00)
        self.assertEqual(response.data['cupon_info']['codigo'], 'VERANO2025')
    ...
    def test_calcula_descuento_monto_fijo_correctamente(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(response.data['valid'])
        self.assertEqual(float(response.data['discount']), 50.00)
        self.assertEqual(response.data['cupon_info']['tipo'], 'monto_fijo')
