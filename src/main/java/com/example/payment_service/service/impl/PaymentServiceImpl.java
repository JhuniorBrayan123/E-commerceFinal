...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentResponse processRefund(Long paymentId, RefundRequest request, String jwtToken) {
        ...
        if (payment.getStatus() != Payment.PaymentStatus.PAID) {
            throw PaymentException.badRequest("INVALID_STATUS", "Solo pagos en estado PAID pueden reembolsarse");
        }

        if (request.getAmount().compareTo(payment.getAmount()) > 0) {
            throw PaymentException.badRequest("INVALID_AMOUNT", "El monto del reembolso excede el pago original");
        }
