...
export const categoriasService = {
  ...
  getProductos: (id: number) => api.get(`/categorias/${id}/sensores/`),
};
...
export const comentariosService = {
  getAll: (id: number) => api.get(`/comentarios/?sensor_id=${id}`),
...
export const carritoService = {
  ...
  add: (producto: any, cantidad: number = 1, stock: number) => {
    ...
    if (existente) {
      ...
      // Crear un objeto normalizado para el carrito
      const itemCarrito = {
        id: producto.id,
        ...
        imagen_url: producto.imagen || null, // Para compatibilidad
...
// Orders Service - Para consultar pedidos del usuario
export const ordersService = {
  getMisPedidos: (userId: number) => api.get(`/mis-pedidos/?user_id=${userId}`),
};
...
export const orderService = {
  createOrder: async (orderData: any) => {
    try {
      ...
      try {
        ...
        console.error(
          "orderService.createOrder - Error parsing JSON:",
          parseError
        );
        throw new Error(
          `Error al parsear la respuesta del servidor: ${text.substring(
            0,
            200
          )}`
        );
      ...
      if (
        ...
        throw new Error(
          "No se pudo conectar con el servicio de pagos. Verifica que el servicio esté corriendo en el puerto 8080."
        );
      }
      ...
      throw new Error(
        "Error de conexión al crear la orden: " +
  ...
  getOrderStatus: async (orderId: number) => {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        error.message || "Error al obtener el estado de la orden"
      );
    }
    return res.json();
  },
...
export const paymentService = {
  confirmPayment: async (confirmData: any) => {
    ...
    if (data.success === false) {
      const errorMessage =
        data.error?.message || data.message || "Error al procesar el pago";
      const error = new Error(errorMessage);
      (error as any).code = data.error?.code;
      (error as any).response = data;
      throw error;
