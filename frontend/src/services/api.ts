...
export const productosService = {
  ...
  porCategoria: (categoriaId: number) => api.get(`/productos/por_categoria/?categoria_id=${categoriaId}`),
...
export const inventarioService = {
  ...
  registrarMovimiento: (data: any) => api.post('/inventario/registrar_movimiento/', data),
  historial: (productoId?: number) => {
    const params = productoId ? { producto_id: productoId } : {};
    return api.get('/inventario/historial/', { params });
  },
  stockActual: (productoId: number) => api.get(`/inventario/stock_actual/?producto_id=${productoId}`),
};
...
export const carritoService = {
  ...
  remove: (productoId: number) => {
    const carrito = carritoService.get().filter((item: any) => item.id !== productoId);
    localStorage.setItem('carrito', JSON.stringify(carrito));
    return carrito;
  },
